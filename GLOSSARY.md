...
| actuator force ceiling | The hard limit on force an actuator may produce, declared as `forcerange` in MJCF; the gripper's is ±3.35 N, so no amount of commanding a tighter squeeze exceeds it (`docs/hardware/grasp-envelope.md`). |
...
| convex hull collision | MuJoCo collapses a `type="mesh"` collision geom to its convex hull unless a decomposition is declared, so a C-shaped part becomes a solid blob — this is why the SO-101's two jaws overlapped at every joint angle (ADR-028). |
...
| finger pad primitive | A small box geom added to each jaw to do the gripping, with the jaw's own mesh collision switched off — the standard MuJoCo answer to convex hull collision, since a box is already convex and cannot be distorted by hulling (ADR-028). |
...
| grasp envelope | The range of object thicknesses a gripper can actually pinch and hold, measured functionally by closing on test boxes rather than inferred from geometry (`docs/hardware/grasp-envelope.md`). |
...
| kinematic offset | The gap between the frame IK aims at and the point that actually does the work; M06 began by solving to the upstream `gripperframe` site ~8 cm from where the jaws pinch, so every solve was accurate to the wrong place. |
...
| reachability envelope | The set of points an arm can actually reach, measured by sweeping a grid of targets through the IK solver rather than assumed from a datasheet reach number; only meaningful when measured from a valid, non-colliding rest pose (`docs/hardware/m06-reachability-probe.md`). |
...
| target-prop exemption | Excluding the object being picked from the arm-vs-prop collision check, since touching it is the point — capped by a crush threshold so a genuine crush is still caught. |
...
| waypoint staging | Splitting one motion into ordered waypoints (approach → descend → grip → retreat), each solved and reached in turn, so a collision-blind IK solver never takes the straight line through the table (ADR-027). |
| weld constraint | A MuJoCo constraint that rigidly fixes two bodies together; the shortcut way to fake a grasp, rejected for this project because it is not really grasping and would need disclosure (ADR-028). |
